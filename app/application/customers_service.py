...
class CustomersService(RetailCRMBaseService):
    ...
    async def get_customers(self, filters: dict[str, Any]) -> list[dict[str, Any]]:
        try:
            resp = await self.client.get(
                path=await self.set_filters(self.CUSTOMERS_PATH, filters)
            )
        ...
        for c in resp.get("customers", []):
            phones = (
                [{"number": p.get("number")} for p in c.get("phones", [])]
                if c.get("phones")
                else None
            )
            result.append(
    ...
    async def create_customer(self, data: dict[str, Any]):
        ...
        form_data = await self.build_form_data(**{"site": site}, **{"customer": data})
        try:
