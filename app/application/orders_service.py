...
class OrdersService(RetailCRMBaseService):
    ...
    async def get_orders_by_customer(self, filters: dict[str, int]) -> list[dict[str, Any]]:
        try:
            resp = await self.client.get(path=await self.set_filters(self.ORDERS_PATH, filters))
    ...
    async def create_order(self, data: dict[str, Any]) -> dict[str, Any]:
        try:
            resp = await self.client.post(self.ORDERS_ADD_PATH,
                                          data=await self.param_encode('order', data))
        except httpx.HTTPStatusError as exc:
            raise HTTPException(status_code=400, detail=exc.response.json())
    ...
    async def create_payment(self, data: dict) -> dict[str, int | bool]:
        try:
            resp = await self.client.post(self.ORDERS_LINK_PAYMENT_PATH,
                                          data=await self.param_encode('payment', data))
        except httpx.HTTPStatusError as exc:
            raise HTTPException(status_code=400, detail=exc.response.json())
